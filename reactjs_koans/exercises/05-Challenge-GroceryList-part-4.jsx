...
class GroceryList extends React.Component {
  ...
  addGroceryItem(newGroceryName) {
    this.setState({
      groceries: this.state.groceries.concat([{name: newGroceryName, completed: false}])
    });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <GroceryForm onAddGroceryItem={ this.addGroceryItem } onClearList={ this.clearList }/>
      </div>
    );
  }
}
...
class GroceryForm extends React.Component {
  ...
  addGroceryItem() {
    if(this.state.newGroceryName) {
      this.props.onAddGroceryItem(this.state.newGroceryName)
    }
  }
  ...
  render() {
    const  newProductInput     = <input  className='new-item'    onChange={this.inputChanged} type="text" />;
    const  newProductAddButton = <button className='add-product' onClick={this.addGroceryItem} disabled={this.state.newGroceryName == ''}>Add new Product</button>;
